...
function openAccount(forceCheckout=false){
 if(account){
  modal.innerHTML=`<button class="close" onclick="closeModal()">✕</button><h2>حساب کاربری</h2><div class="account-box"><div class="account-status">👤 <b>${account.name}</b><br><small>${account.rubika||"آیدی ثبت نشده"}</small></div><div class="notice">این حساب فقط در حافظه موقت همین صفحه نگه داشته می‌شود؛ با بستن/رفرش صفحه از بین می‌رود.</div><button class="btn secondary" onclick="logout()">خروج از حساب</button></div>`;
  openModal(); return;
 }
 modal.innerHTML=`<button class="close" onclick="closeModal()">✕</button><h2>ورود سریع</h2>
 <div class="notice">برای اینکه سایت بدون هاست و دیتابیس هم کار کند، حساب کاربری این نسخه فقط در حافظه موقت مرورگر نگه داشته می‌شود و در هیچ جایی ذخیره نمی‌شود. با Refresh یا بستن صفحه پاک می‌شود.</div>
